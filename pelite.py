...
def banner():
    print("\n%s    ______ __     \n%s   / _ (_/____  ║ %sDesign By %sNdraa %s& %sMas Ndra\n%s  /|||||/ / / ___ \ ║ %sFB %s: %sFacebook.com/1000xxxx\n%s / /_~~/ / /_/  __/ ║ %sFB %s: %sndraa x reall\n%s/_____/<><>\__/\_/  ║ %sGithub %s: %sGithub.com/Ndraaa\n"%(O,O,P,O,P,O,O,P,O,P,O,P,O,P,O,P,O,P))
def menu_log():
...
def menu():
    ...
    try:
        ...
        users = 'Indra x Real'
        maile = 'cnnindosiar2@gmail.com'
        kunci = 'KUNCI RUMAH '
        bergabung = '11-30-2021'
        kadaluarsa = 'Gk kadaluwarsa'
        pro = ("%s[%sPro%s]"%(O,P,O))
        upgrade = ('Upgrade To Version %sFree'%(O))
        jid = ('%s[%s7000 ID%s]'%(O,P,O))
    try:
    ...
    print('%s '%(O))
    print('%s [%s1%s] %sCrek seko publik %s'%(O,P,O,P,jid))
    print('%s [%s2%s] %sCrak tekan seng meluni kue%s'%(O,P,O,P,jid))
    print('%s [%s3%s] %sCrak Tekan like %s'%(O,P,O,P,jid))
    print('%s [%s4%s] %sJimok data Target'%(O,P,O,P))
    print('%s [%s5%s] %sCrak tekan nomor tlp %s'%(O,P,O,P,pro))
    print('%s [%s6%s] %sCek hasil krek mu' %(O,P,O,P))
    print('%s [%s7%s] %sCek opsi akun fb ne%s'%(O,P,O,P,pro))
